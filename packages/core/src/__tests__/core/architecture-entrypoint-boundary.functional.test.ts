...
describe('architecture quick start entrypoint boundary', () => {
  ...
  test('documents the helper split behind the programmatic and web entrypoints', () => {
    ...
    expect(architectureQuickStart).toContain('The shared standalone runner resolves `require.main` in one place through `resolveStandaloneEntrypointMainModule()` so wrapper guards do not each reinvent default main-module lookup.');
