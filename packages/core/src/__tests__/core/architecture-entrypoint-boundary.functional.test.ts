...
describe('architecture quick start entrypoint boundary', () => {
  ...
  test('documents the helper split behind the programmatic and web entrypoints', () => {
    ...
    expect(architectureQuickStart).toContain('CLI default API/WS ports are static runtime constants in `packages/core/src/cli/cli-runtime.ts`; `API_PORT` and `WS_PORT` override only those embedded defaults.');
