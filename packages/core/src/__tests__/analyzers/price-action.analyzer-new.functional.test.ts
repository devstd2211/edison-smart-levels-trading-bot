...
describe('PriceActionAnalyzerNew - Functional: Snapshot Semantics', () => {
  it('should keep state snapshots isolated across analyses', () => {
    const analyzer = new PriceActionAnalyzerNew(createConfig());

    const firstSignal = analyzer.analyze(createCandles(Array.from({ length: 24 }, (_, i) => 100 + i * 0.3)));
    ...
    const secondSignal = analyzer.analyze(createCandles(Array.from({ length: 24 }, (_, i) => 120 - i * 0.3)));
