 ...
 *    - OPEN: Check TP1 hit to move to TP1_HIT (move SL to breakeven)
 *    - TP1_HIT: Check TP2 hit to move to TP2_HIT (activate trailing)
 *    - TP2_HIT: Check TP3 hit to move to TP3_HIT (close remaining)
...
export function evaluateExit(context: ExitDecisionContext): ExitDecisionResult {
  ...
  if (validationError) {
    return {
      ...
      stateTransition: `ERROR to CLOSED (${validationError})`,
    };
  }
  ...
  // =====================================================================
  // STEP 1: Check if Stop Loss hit (ANY state to CLOSED) - PRIORITY 1
  // =====================================================================
  if (checkStopLossHit(position, currentPrice)) {
    ...
    return {
      ...
      stateTransition: `${currentState} to CLOSED (SL HIT)`,
      metadata: {
  ...
  if (!isValidState(currentState)) {
    return {
      ...
      stateTransition: `INVALID to CLOSED`,
    };
  }
  ...
  if (currentState === PositionState.OPEN) {
    ...
    if (checkTPHit(position, currentPrice, 0)) {
      ...
      return {
        ...
        stateTransition: `OPEN to TP1_HIT`,
        metadata: {
    ...
    if (checkTPHit(position, currentPrice, 1)) {
      ...
      return {
        ...
        stateTransition: `TP1_HIT to TP2_HIT`,
        metadata: {
    ...
    if (checkTPHit(position, currentPrice, 2)) {
      ...
      return {
        ...
        stateTransition: `TP2_HIT to TP3_HIT`,
        metadata: {
    ...
    return {
      ...
      stateTransition: `TP3_HIT to HOLDING`,
    };
  }
  ...
  return {
    ...
    stateTransition: `${currentState} to NO_CHANGE`,
  };
}
