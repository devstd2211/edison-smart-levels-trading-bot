...
const WHALE_DETECTOR_FOLLOW_THRESHOLDS = {
  ...
  // Confidence calculation factors (same as regular whale detector)
...
export class WhaleDetectorFollowService {
  ...
  detectWhale(analysis: OrderBookAnalysis, currentPrice: number): WhaleSignal {
    ...
    if (Math.random() < WHALE_DETECTOR_FOLLOW_THRESHOLDS.LOG_DETECTION_PROBABILITY) {
    ...
    if (Math.random() < WHALE_DETECTOR_FOLLOW_THRESHOLDS.LOG_NO_DETECTION_PROBABILITY) {
    ...
    for (const [wallPrice, wall] of this.trackedBidWalls.entries()) {
      ...
      if (timeSinceLastSeen > wallGoneThresholdMs) {
        ...
        return {
          ...
          reason: `BID wall DISAPPEARED @ ${wallPrice.toFixed(DECIMAL_PLACES.PRICE)} (existed ${(wallLifetime / INTEGER_MULTIPLIERS.ONE_THOUSAND).toFixed(0)}s) - Accumulation done, distribution likely`,
          metadata: {
            wall,
          },
        };
    ...
    for (const [wallPrice, wall] of this.trackedAskWalls.entries()) {
      ...
      if (timeSinceLastSeen > wallGoneThresholdMs) {
        ...
        return {
          ...
          direction: SignalDirection.LONG,
          confidence: this.calculateDisappearanceConfidence(wall, wallLifetime),
          reason: `ASK wall DISAPPEARED @ ${wallPrice.toFixed(DECIMAL_PLACES.PRICE)} (existed ${(wallLifetime / INTEGER_MULTIPLIERS.ONE_THOUSAND).toFixed(0)}s) - Distribution done, accumulation likely`,
          metadata: {
            wall,
          },
        };
  ...
  private cleanupExpiredData(): void {
    ...
    if (this.recentlyBrokenWalls.size > WHALE_DETECTOR_FOLLOW_THRESHOLDS.RECENT_BREAKS_MAX_SIZE) {
  ...
  private calculateBreakConfidence(wall: WhaleWall): number {
    ...
    const sizeScore = Math.min(
      (wall.percentOfTotal / WHALE_DETECTOR_FOLLOW_THRESHOLDS.BREAK_SIZE_SCORE_DIVISOR) * WHALE_DETECTOR_FOLLOW_THRESHOLDS.BREAK_SIZE_SCORE_MULTIPLIER,
      WHALE_DETECTOR_FOLLOW_THRESHOLDS.BREAK_SIZE_SCORE_MAX,
    );

    // Factor: Distance (closer break = higher confidence)
    const distanceScore = Math.max(
      WHALE_DETECTOR_FOLLOW_THRESHOLDS.BREAK_DISTANCE_BASE - wall.distance * WHALE_DETECTOR_FOLLOW_THRESHOLDS.BREAK_DISTANCE_MULTIPLIER,
      WHALE_DETECTOR_FOLLOW_THRESHOLDS.BREAK_DISTANCE_MIN,
    );
  ...
  private calculateDisappearanceConfidence(wall: WhaleWall, wallLifetime: number): number {
    // Factor: Wall size
    const sizeScore = Math.min(
      (wall.percentOfTotal / WHALE_DETECTOR_FOLLOW_THRESHOLDS.DISAPPEARANCE_SIZE_DIVISOR) * WHALE_DETECTOR_FOLLOW_THRESHOLDS.DISAPPEARANCE_SIZE_MULTIPLIER,
      WHALE_DETECTOR_FOLLOW_THRESHOLDS.DISAPPEARANCE_SIZE_MAX,
    );

    // Factor: Lifetime (longer = higher confidence)
    const lifetimeScore = Math.min(
      (wallLifetime / WHALE_DETECTOR_FOLLOW_THRESHOLDS.DISAPPEARANCE_LIFETIME_DIVISOR) * WHALE_DETECTOR_FOLLOW_THRESHOLDS.DISAPPEARANCE_LIFETIME_MULTIPLIER,
      WHALE_DETECTOR_FOLLOW_THRESHOLDS.DISAPPEARANCE_LIFETIME_MAX,
    );
  ...
  private calculateSpikeConfidence(ratioChange: number): number {
    ...
    const confidence = Math.min(
      (ratioChange - 1) * WHALE_DETECTOR_FOLLOW_THRESHOLDS.SPIKE_CONFIDENCE_MULTIPLIER,
