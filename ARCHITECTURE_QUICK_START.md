...
- `@edison/core/config` stays on `packages/core/src/config/index.ts`; it is the dedicated config-only surface for runtime-config helpers and the publishable `ConfigPipelineLoader` type.
...
2. Config-only consumers can stay on `@edison/core/config`, while `@edison/core/core` keeps a type-only convenience re-export for the same loader contract.
3. Config-aware helpers can load validated runtime config without going through the legacy root wrapper, and `loadBotRuntimeConfig(loader?)` stays as the shared public config-loader seam for `createConfiguredBot()`, `createConfiguredBotRuntime()`, and `startConfiguredBot()`.
