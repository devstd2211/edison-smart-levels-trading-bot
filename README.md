...
- CLI configuration summary, test-mode, fatal startup, endpoint, and warning output stay behind exported constants in `packages/core/src/cli/cli-entrypoint-runtime.ts`; port parsing stays behind the named `parseCliPort(...)` helper in `packages/core/src/cli/cli-runtime.ts`.
