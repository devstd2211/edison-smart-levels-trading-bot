...
- Completed a lifecycle/testability and suite-state reduction follow-up for `exit-type-detector.service`, `funding-rate-filter.service`, `health-check`, `indicator-cache.error-handling`, `indicator-precalculation.error-handling`, and `indicator-registry.error-handling`.
  - removed the remaining binder/getter wrapper layers from those suites so setup now exposes only the narrower helper-managed fixture groups each test actively exercises.
  - confirmed that the previously listed `indicator-cache.repository-integration` candidate is currently archived as `.bak`; used the active `indicator-cache.error-handling` suite instead.
  - reviewed the adjacent production services for safe follow-up refactors; none were required in this slice.
- Completed a lifecycle/testability and suite-state reduction follow-up for `delta-analyzer.service`, `dynamic-position-sizer`, `entry-confirmation.service`, `event-deduplication.service`, `exchange-factory.service`, and `graceful-shutdown.service`.
  - removed the remaining binder/getter wrapper layers from those suites so setup now exposes only the narrower helper-managed fixture groups each test actively exercises.
  - reviewed the adjacent production services for safe follow-up refactors; none were required in this slice.
- Completed a lifecycle/testability and suite-state reduction follow-up for `advanced-order-state-machine.service`, `analyzer-registration-fixes`, `bybit.repository-integration`, `candle-provider.repository-integration`, `bot-factory.service`, and `monitoring-server`.
  - removed the remaining binder/getter wrapper layers and intermediate fixture bundle naming in those suites so setup now exposes the narrower helper-managed fixture groups each test actually exercises.
  - reviewed the adjacent production services/providers; no safe behavior-preserving production refactor was required in this slice, and the analyzer-registration coverage remains helper/config-owned.
- Completed a lifecycle/testability and suite-state reduction follow-up for `bot-factory.error-handling`, `create-services.lifecycle`, `health-check`, `time.service`, `websocket-keep-alive.service`, and `signal-processing.timeframe-conflict`.
  - removed the remaining binder/getter wrapper layers from those suites so setup now exposes only the helper-managed lifecycle or fixture state each test actively exercises.
  - collapsed repeated inline `TrendAnalysis` construction in the timeframe-conflict tests into narrow local builders/constants to reduce suite-local scenario noise without changing assertions.
  - reviewed the adjacent production services/utilities for safe follow-up refactors; none were required in this slice.
...
- Verification:
  ...
  - `npm test -- --runInBand packages/core/src/__tests__/services/advanced-order-state-machine.test.ts packages/core/src/__tests__/services/analyzer-registration-fixes.test.ts packages/core/src/__tests__/services/bybit.repository-integration.test.ts packages/core/src/__tests__/services/candle-provider.repository-integration.test.ts packages/core/src/__tests__/services/bot-factory.service.test.ts packages/core/src/__tests__/services/monitoring-server.test.ts` -> PASS.
  - `npm test -- --runInBand packages/core/src/__tests__/services/bot-factory.error-handling.test.ts packages/core/src/__tests__/services/create-services.lifecycle.test.ts packages/core/src/__tests__/services/health-check.test.ts packages/core/src/__tests__/services/time.service.test.ts packages/core/src/__tests__/services/websocket-keep-alive.service.test.ts packages/core/src/__tests__/services/signal-processing.timeframe-conflict.test.ts` -> PASS.
...
- Favor the next remaining slices that still keep full helper contexts, inline temporary managed contexts, wider factory state, or optional cleanup wrappers in scope even though their lifecycle ownership is already centralized, with `ladder-tp-manager.service`, `limit-order-executor.service`, `ml-feature-extractor.service`, `position-monitor.service`, `position-pnl-calculator.service`, and `real-time-risk-monitor.service` as the next easy cleanup candidates, and separately decide whether `packages/core/src/__tests__/services/phase-10-integration.test.ts` should keep its current memory-growth threshold or be stabilized with a less environment-sensitive assertion before continuing adjacent suite cleanup.
