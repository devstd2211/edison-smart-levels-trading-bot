...
- Completed the `strategy-config-merger` + `strategy-loader` + `ml-feature-extractor` testability follow-up batch:
  - added shared `strategy-config-merger` helpers with explicit service and `ErrorHandler` factories plus reusable default main-config and strategy fixtures, then aligned the error-handling suite on those helpers instead of repeating direct `StrategyConfigMergerService` construction.
  - added shared `strategy-loader` helpers with reusable mocked `ErrorHandler` bootstrap plus explicit service factory wiring for temporary strategy directories, then aligned the error-handling suite on those helpers instead of repeating direct `StrategyLoaderService` construction.
  - extended the shared `ml-feature-extractor` helpers with an explicit service factory that preserves no-handler bootstrap paths, then aligned the remaining logging-failure and backward-compatibility cases on helper-driven construction instead of repeating direct `MLFeatureExtractorService` construction.
  - reviewed `services/strategy-config-merger.service.ts`, `services/strategy-loader.service.ts`, and `services/ml-feature-extractor.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
