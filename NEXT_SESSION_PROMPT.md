...
- Completed the `liquidity-heatmap` testability follow-up slice:
  - added shared helpers for canonical logger/ErrorHandler bootstrap, typed config/orderbook coercion, and reusable normal/thin/deep orderbook fixtures, then aligned the error-handling suite on that harness instead of repeating local logger/config/service/orderbook builders.
  - reviewed `services/liquidity-heatmap.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
