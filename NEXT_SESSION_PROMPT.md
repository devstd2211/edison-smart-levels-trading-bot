...
- Completed the `risk-calculator` testability follow-up slice:
  - added shared helpers for canonical mock logger/ErrorHandler bootstrap plus reusable default input and take-profit builders, then aligned the error-handling suite on that harness instead of repeating inline mock logger state and default calculation inputs.
  - reviewed `services/risk-calculator.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
