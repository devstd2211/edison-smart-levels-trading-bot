...
1. `packages/core/src/services/bot-initializer/bot-initializer-shutdown.utils.ts initializer runtime shutdown utility boundary follow-up`
2. `packages/core/src/__tests__/services/bot-initializer-shutdown.utils.test.ts initializer runtime shutdown utility guardrail follow-up`
3. `packages/core/src/__tests__/helpers/websocket-authentication-test.utils.ts websocket authentication runtime fixture boundary follow-up`
...
- `bot-initializer-test.utils.ts` now resolves managed runtime state through live getters and a shared builder path, so `rebuild()` updates remain visible to callers instead of leaking stale snapshots.
- `bot-initializer-retry.utils.ts` now honors shared `RetryConfig.customBackoff` while preserving capped delay behavior, so initializer retry policy matches the central error-handler contract.
- `bot-initializer-retry.utils.test.ts` now guards both capped exponential and custom-backoff delay paths, which makes future retry-boundary drift visible immediately.

## Last Verification
- `npm test -- --runInBand packages/core/src/__tests__/bot-initializer.test.ts packages/core/src/__tests__/services/bot-initializer.functional.test.ts packages/core/src/__tests__/services/bot-initializer.error-handling.test.ts packages/core/src/__tests__/services/bot-initializer-retry.utils.test.ts packages/core/src/__tests__/helpers/bot-initializer-test.utils.test.ts`
