...
- Completed the `enhanced-exit` testability follow-up slice:
  - added shared helpers for canonical logger/ErrorHandler bootstrap and reusable default config wiring, then aligned the error-handling suite on that harness instead of repeating local mock logger creation and service construction across validation, integration, update-config, edge-case, and backward-compat blocks.
  - reviewed `services/enhanced-exit.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
