...
- Completed the `analyzer-engine` service-suite testability follow-up slice:
  - extended the shared helpers with a reusable service harness, and aligned the main service suite on the same builders already used by the error-handling slices instead of maintaining a second local copy of analyzer/registry/config/candle/logger factories.
  - reviewed `services/analyzer-engine.service.ts` again; kept production code unchanged because this batch only needed further test-harness consolidation.
- Verification:
