...
- Completed 3-slice batch in session-stats entry analytics decomposition:
  - extracted `recordSessionTradeEntryWithSkip(...)`
  - extracted `logSessionTradeEntrySkipResult(...)`
  - simplified `recordSessionTradeEntryWithResilience(...)` orchestration
  with unchanged `openPosition.recordTradeEntry` skip behavior.
- Completed 3-slice batch in snapshot fallback decomposition:
  - extracted `getPositionSnapshotWithErrorHandler(...)`
  - extracted `getPositionSnapshotWithoutErrorHandler(...)`
  - simplified `getPositionSnapshot()` orchestration
  with unchanged fallback/degraded behavior.
- Completed 3-slice batch in websocket-restore decomposition:
  - extracted `restorePositionFromWebSocketWithJournal(...)`
  - extracted `restorePositionFromWebSocketWithoutJournal(...)`
  - simplified `restorePositionFromWebSocketSync(...)` branching
  with unchanged restore and degraded behavior.
- Completed 5-slice batch in analytics/websocket orchestration decomposition:
  - extracted `recordSessionTradeEntryForOpen(...)`
  - extracted `shouldRecordSessionTradeEntry(...)`
...
- Completed 8-slice error-message normalization in logging helpers:
...
- Completed 7-slice batch in open/analytics direct-path decomposition:
  ...
  with unchanged behavior.
- Completed 5-slice batch in pre-open resilience branch decomposition:
