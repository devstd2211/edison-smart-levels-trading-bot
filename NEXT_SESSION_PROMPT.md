...
## Next Step
- Continue iteration-2 on `packages/core/src/services/position-lifecycle.service.ts`: apply structure-only cleanup for websocket/snapshot helper grouping and section comments, then run the same targeted lifecycle suites and update `REFACTOR_PLAN.md`.
