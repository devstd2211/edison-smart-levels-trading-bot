...
- Completed the `whale-detection` testability follow-up slice:
  - extended the shared helpers through the error-handling suite so config/logger/service construction now flows through the same harness already used by adjacent tests instead of repeating local detector bootstrap and backward-compat setup.
  - reviewed `services/whale-detection.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
