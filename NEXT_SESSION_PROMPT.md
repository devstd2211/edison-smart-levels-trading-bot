...
- Completed the `position-sync` + `limit-order-executor` + `data-collector` testability follow-up batch:
  - extended the shared `position-sync` helpers with override-based harness creation so custom close-recorder and no-handler constructor paths can reuse the canonical service factory instead of repeating direct `PositionSyncService` construction.
  - added dedicated `limit-order-executor` and `data-collector` helper files with explicit service factories for canonical config/logger/dependency/ErrorHandler bootstrap, then aligned their remaining constructor-heavy error-handling scenarios on those helper surfaces instead of repeating direct service construction.
  - reviewed `services/position-sync.service.ts`, `services/limit-order-executor.service.ts`, and `services/data-collector.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
