...
## Next Step
- Continue iteration-2 on `packages/core/src/services/position-lifecycle.service.ts`: extract next behavior-safe block from sizing-path log payload shaping (`compound`/`kelly` success/fallback logs), then run the same targeted lifecycle suites and update `REFACTOR_PLAN.md`.
