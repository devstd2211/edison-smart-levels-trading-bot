...
- Completed the `whale-wall-tp` testability follow-up slice:
  - added shared helpers for canonical logger/ErrorHandler bootstrap, reusable wall fixtures, and nested config overrides, then aligned the error-handling suite on that harness instead of repeating local service/config/logger setup across validation, integration, and backward-compat blocks.
  - reviewed `services/whale-wall-tp.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
