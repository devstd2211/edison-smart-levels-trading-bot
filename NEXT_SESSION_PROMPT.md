...
- Completed the `candle-provider.error-handling` + `websocket-manager.error-handling` + `websocket-event-handler.error-handling` testability follow-up batch:
  - continued collapsing repeated `candle-provider.error-handling` bootstrap behind a local scenario builder plus the existing shared harness for the repeated RETRY/initialize slices, extended the shared `websocket-manager` helper with explicit ErrorHandler/service factories for canonical bootstrap reuse, and extended the shared `websocket-event-handler` helper with an explicit handler factory so fresh-instance coverage no longer depends on inline constructor wiring.
  - reviewed `providers/candle.provider.ts`, `services/websocket-manager.service.ts`, and `services/handlers/websocket.handler.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
