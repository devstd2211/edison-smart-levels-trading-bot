...
- Completed the `anomaly-detection` testability follow-up slice:
  - added shared helpers for canonical logger/ErrorHandler bootstrap, mock-logger variants, reusable trade fixtures, and history seeding, then aligned the error-handling suite on that harness instead of repeating local service construction and inline trade/logger setup.
  - reviewed `services/anomaly-detection.service.ts`; kept production code unchanged because this batch only needed test-harness consolidation.
- Verification:
