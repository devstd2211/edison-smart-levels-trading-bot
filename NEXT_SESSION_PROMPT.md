...
- Completed a lifecycle/testability and suite-state reduction follow-up for `event-handlers.error-handling`, `exchange-factory.error-handling`, `graceful-shutdown.error-handling`, `ladder-exit-detector.service.error-handling`, `ml-feature-extractor.error-handling`, and `trading-journal.error-handling`.
  - removed the remaining binder/getter wrapper layers so setup now exposes only the narrower helper-managed runtime/factory bundles each suite actively exercises.
...
- Favor the next remaining slices that still keep full helper contexts, inline temporary managed contexts, wider factory state, or optional cleanup wrappers in scope even though their lifecycle ownership is already centralized; nearby candidates now include suites such as `indicator-cache.error-handling`, `indicator-precalculation.error-handling`, `market-condition-analyzer.error-handling`, `mtf-snapshot-gate.error-handling`, `risk-calculator.error-handling`, and `strategy-loader.error-handling`, while separately deciding whether `packages/core/src/__tests__/services/phase-10-integration.test.ts` should keep its current memory-growth threshold or be stabilized with a less environment-sensitive assertion before continuing adjacent suite cleanup.
