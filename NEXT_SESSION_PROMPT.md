...
- Completed the next lifecycle/testability and suite-state reduction follow-up for `analyzer-engine.error-handling-advanced`, `analyzer-engine.error-handling`, `analyzer-engine.service`, `bot-initializer.error-handling`, `compound-interest-calculator.service`, `compound-interest-calculator.error-handling`, `config-validator.error-handling`, `console-dashboard.error-handling`, `circuit-breaker.service`, and `circuit-breaker.error-handling`.
  - replaced direct exported `Managed*Context` type dependence and narrow `Pick<Managed...>` aliases with local runtime aliases derived from `ReturnType<typeof createManaged...>`, narrowed helper-owned suite state, and kept the slice test-only and behavior-preserving.
