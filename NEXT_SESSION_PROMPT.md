...
- Completed another three-iteration orderbook/position-sync helper-consolidation batch:
  - extended `packages/core/src/__tests__/helpers/position-sync-test.utils.ts` with shared position-age and protection-order builders and reused them across both `position-sync` suites instead of repeated local position/order factories.
  - routed `packages/core/src/__tests__/services/orderbook-manager.service.test.ts` through the shared legacy service builder instead of direct inline legacy bootstrap.
  - reviewed `services/position-sync.service.ts` and `services/orderbook-manager.service.ts`; left production code unchanged after review.
- Verification:
  - `npm test -- --runInBand packages/core/src/__tests__/services/orderbook-manager.service.test.ts packages/core/src/__tests__/services/position-sync.service.test.ts packages/core/src/__tests__/services/position-sync.service.error-handling.test.ts` -> PASS (3/3 suites, 62/62 tests).
...
- Likely next candidates: `orderbook-manager` and `position-sync` are now largely normalized, so move to the next adjacent constructor-heavy untouched suite such as `position-state-machine.error-handling`, `position-pnl-calculator.error-handling`, or another nearby test still rebuilding local harness state.
