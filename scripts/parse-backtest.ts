...
export function analyzeBacktest(result: BacktestResult): void {
  ...
  console.log('\n═══════════════════════════════════════════════════════════════');
  console.log(`${ICONS.chart} BACKTEST ANALYSIS`);
  console.log('═══════════════════════════════════════════════════════════════\n');

  // Key metrics
  console.log(`${ICONS.chart_up} PERFORMANCE:`);
  console.log(`  Total Trades: ${s.totalTrades}`);
  console.log(`  ${ICONS.success} Wins: ${s.winningTrades} (${s.winRate.toFixed(2)}%)`);
  console.log(`  ${ICONS.error} Losses: ${s.losingTrades} (${(100 - s.winRate).toFixed(2)}%)`);
  console.log();

  console.log(`${ICONS.money} PROFITABILITY:`);
  ...
  console.log(`${ICONS.balance} RISK/REWARD:`);
  ...
  console.log(`${ICONS.chart_down} RISK METRICS:`);
  ...
  if (s.netPnl < 0) {
    console.log(`  ${ICONS.error} LOSING STRATEGY (Net: ${s.netPnl.toFixed(2)} USDT)`);
  } else if (s.netPnl > 0) {
    console.log(`  ${ICONS.success} Profitable (Net: +${s.netPnl.toFixed(2)} USDT)`);
  }

  if (s.winLossRatio < 1.5) {
    console.log(`  ${ICONS.warning}  W/L Ratio too low (${s.winLossRatio.toFixed(2)}:1, target: 2:1+)`);
    console.log(`      Average loss is ${(s.avgLoss / s.avgWin).toFixed(1)}x larger than avg win`);
  }

  if (s.totalFees > s.totalPnl) {
    console.log(`  ${ICONS.warning}  Fees (${s.totalFees.toFixed(2)}) exceed gross profit (${s.totalPnl.toFixed(2)})`);
  }

  if (s.maxDrawdownPercent > 20) {
    console.log(`  ${ICONS.warning}  Drawdown too high (${s.maxDrawdownPercent.toFixed(2)}%, target: <10%)`);
  }
  ...
  if (losingTrades.length > 0) {
    console.log(`${ICONS.pin} LOSING TRADES ANALYSIS:`);
    ...
    if (veryBadTrades.length > 0) {
      console.log(`  ${ICONS.red_circle} Very bad trades (>-2%): ${veryBadTrades.length}`);
...
async function main() {
  ...
  if (!fs.existsSync(filePath)) {
    console.error(`${ICONS.error} File not found: ${filePath}`);
    process.exit(1);
  }

  console.log(`${ICONS.open_folder} Parsing: ${path.basename(filePath)}`);
