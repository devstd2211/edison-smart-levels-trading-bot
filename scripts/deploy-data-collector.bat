@echo off
REM Deploy Data Collector (Multi-Symbol) to dedicated folder
REM Symbols: APEXUSDT, SUIUSDT, STRKUSDT, HYPEUSDT, ADAUSDT, XLMUSDT
REM Purpose: Collect orderbook + candles + ticks data for calibration

echo ========================================
echo Deploying Data Collector (Multi-Symbol)
echo ========================================

set SOURCE_DIR=D:\src\Edison
set DEPLOY_DIR=D:\src\Edison - data-collector
...
REM Copy custom config for Data Collector
copy /Y "%SOURCE_DIR%\configs\config-data-collector.json" "%DEPLOY_DIR%\config.json"
...
echo ========================================
echo.
echo Purpose: Multi-Symbol Data Collection
echo Symbols: APEXUSDT, SUIUSDT, STRKUSDT, HYPEUSDT, ADAUSDT, XLMUSDT
echo Location: %DEPLOY_DIR%
echo Database: %DEPLOY_DIR%\data\market-data-multi.db
echo.
...
echo   - Candles (all timeframes)
echo   - Orderbook snapshots (every 1s)
echo   - Trade ticks
echo.
echo Press Ctrl+C to stop collecting data
echo.
pause
