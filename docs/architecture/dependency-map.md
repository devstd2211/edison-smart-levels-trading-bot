...
```text
Legacy wide service state (being phased out)
|
+-- IMarketDataServices
|   +-- candleProvider
|   +-- orderbookManager
...
Pattern: grouped services instead of one flat legacy service state.
...
| Runtime Service Adapters | `IBotRuntimeSource` | done |
...
- Completed components: 128
- Active queue: 5
- Latest completed: `Legacy BotServices wording retirement in tests/docs`
- Next target area: runtime contract wording cleanup and adapter/module naming follow-up
...
- Do not pass the full legacy service state into components that only need a few dependencies.
...
**Auto-generated by:** Session 122 runtime naming cleanup
